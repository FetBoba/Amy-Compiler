...
object AllPrecedence {
  val result: Boolean = !false && 1 < 2 + 3 * 4 || 5 == 6;
  result
}
...
object LegalNesting {
  if (true) { val x: Int = 1; x } else { 2 };
  val y: Int = if (false) { 3 } else { 4 };
  y
}
object ValAsOperand {
  val x: Int = 2; x + 1 
...
object ComplexExpression {
  ...
  if (x < y) {
    val z: String = "x is less than y";
    Std.printString(z);
    x
  } else {
    val w: String = "x is not less than y";
    Std.printString(w);
    y
  }
}
...
object NestedMatch2 {
  ...
  def eval(e: Expr): Int = {
    e match {
      ...
      case Mul(e1, e2) => e1 match {
        case Num(n) => n * eval(e2)
        case _ => e2 match {
          case Num(n) => eval(e1) * n
...
object EmptyBodyMatch {
  ...
  val result: Int = r match {
    case Success(v) => v
    case Failure() => error("Operation failed"); 0
  } ; 1
}
object ModuleA {
  abstract class Tree
...
object QualifiedMatch {
  def process(t: ModuleA.Tree): Int = {
    t match {
      case ModuleA.Leaf(v) => v
      case ModuleA.Node(l, r) => 1
    }
  }
}
object DeepMatching {
  ...
  def countNodes(t: Tree): Int = {
    t match {
      case Leaf(_) => 1
      case Node(Leaf(_), Leaf(_)) => 3
      case Node(Node(_, _), Leaf(_)) => 4
      case Node(Leaf(_), Node(_, _)) => 4
      case Node(Node(_, _), Node(_, _)) => 5
    }
  }
}
object NestedWildcards {
  abstract class Pair
  case class IntPair(a: Int, b: Int) extends Pair
  case class NestedPair(p1: Pair, p2: Pair) extends Pair
  
  def analyze(p: Pair): String = {
    p match {
      case IntPair(a, b) => "IntPair"
      case NestedPair(IntPair(_, _), _) => "Nested with IntPair first"
      case NestedPair(_, IntPair(_, _)) => "Nested with IntPair second"
      case NestedPair(_, _) => "Fully nested"
    }
  }
}
...
object MatchWithSequence {
  ...
  case class Calculate(a: Int, b: Int) extends Command
...
object MultipleMatchesSameValue {
  ...
  def process(t: Tree): Int = {
    ...
    val value: Int = t match {
      case Leaf(v) => v
      case Node(l, r) => process(l) + process(r)
    };
