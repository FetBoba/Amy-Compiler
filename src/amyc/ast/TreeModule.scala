...
trait TreeModule { self =>
  ...
  enum Type:
    ...
    case StringType
    case UnitType
    case ClassType(qname: QualifiedName, typeArgs: List[TypeTree] = Nil) // Added: introduced type arguments
    ...
    override def toString: String = this match
      ...
      case UnitType => "Unit"
      case ClassType(qname, targs) => // Added: handle type arguments 
        val base = qname.toString
        if (targs.nonEmpty)
          s"$base[${targs.map(_.toString).mkString(", ")}]"
        else
          base 
