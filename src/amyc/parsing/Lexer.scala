...
object Lexer extends Pipeline[List[File], Iterator[Token]]
  ...
  val lexer = Lexer(
    ...
    many1(elem(_.isDigit))
      |> { (cs, range) => try {IntLitToken(cs.mkString.toInt).setPos(range._1)} 
        catch {case _: NumberFormatException => ErrorToken("Error: integer overflow: " ++ cs.mkString).setPos(range._1)}
      },
    // TODO: String literals
    elem('"') ~ many(elem(c => c != '"' && c != '\n')) ~ opt(elem('"'))
      |> { (cs, range) => if (cs.mkString.endsWith("\"")) {StringLitToken(cs.mkString.drop(1).dropRight(1)).setPos(range._1)}
        else { ErrorToken("Error: unclosed string literal: " ++ cs.mkString).setPos(range._1) }},
    ...
    word("/*") ~ many(elem(_ != '*') | (elem('*') ~ elem(_ != '/')))
      |> { (cs, range) => ErrorToken("Error: unclosed multiline comment: " ++ cs.mkString).setPos(range._1) },
  ) onError {
